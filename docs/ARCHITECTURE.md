...
- **입사 추천 자동 알림**(`app/services/hiring.py`): 입사 조건을 코드에 명시해두고 6시간마다
  확인하다가 다 채워지면 텔레그램으로 먼저 추천 카드를 보낸다(버튼으로 즉시 입사, 콜백은 기존
  `{target_type}:{id}:{decision}` 3단 구조에 `hire` 타입을 추가해 재사용). 같은 추천은 한 번만
  보낸다(`employees.hire_recommended_at`, migration 0026). 직원 화면에는 조건별 진행률이 항상
  보이므로 CEO가 기준을 외우고 있을 필요가 없다 — 퍼포먼스 마케터 기준은 실제 게시 6건 /
  제품 2개 / 첫 게시 후 14일이고, 조건 전이라도 CEO가 원하면 직접 입사시킬 수 있다.
